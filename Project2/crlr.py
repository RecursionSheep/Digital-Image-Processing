...
size = train.shape[0]
train = train / np.max(train, axis = 0, keep_dims = True)
...
y_in = torch.tensor(train_label, dtype = torch.int)
...
for treatment in range(2048):
	...
	loss += lambda1 * (torch.norm(bal1 - bal2) ** 2)

loss += lambda2 * (torch.norm(W) ** 2) + lambda3 * (torch.norm(beta) ** 2) + lambda4 * torch.norm(W, p = 1)
loss += lambda5 * ((torch.sum(W) - 1) ** 2)
